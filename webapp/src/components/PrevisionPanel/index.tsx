...
import NoBusFeedback from "../NoBusFeedback";
...
const PrevisionPanel: FunctionComponent<PrevisionPanelProps> = ({
  previsions,
  stop
}) => {
  ...
  function getBusRemainingTime(bus: Bus) {
    const [predictedHour, predictedMinutes] = (
      bus.predictedTime || "00:00"
    ).split(":");
  ...
  return (
    <div style={{ maxHeight: panelHeight }} className="stop-previsions-panel">
      <div
        style={{ height: panelHeaderHeight }}
        className="stop-previsions-panel-header"
      >
        <img src={StopIcon} alt="ícone parada de ônibus" />
        <h2>
          Parada <strong>{stop.name}</strong>
        </h2>
      </div>
      <Tabs
        size="small"
              ...
              <div className="stop-previsions-buses">
                <div style={{ height: 90 }}>
                  <h3 className="stop-previsions-bounds">
                    <span className="stop-previsions-bound">
                      {busLine.terminal1}
                    </span>{" "}
                    <br />
                    <span className="stop-previsions-bound">
